...
function RowMenu({ onEdit, onDelete }) {
  ...
  const items = [
    { label: 'تعديل الخدمة', action: () => { onEdit();   setOpen(false) } },
    { label: 'حذف الخدمة',   action: () => { onDelete(); setOpen(false) }, danger: true },
  ]
...
export default function ServiceTable({ services, loading = false, onRefresh }) {
    ...
    <>
      <div className="panel">
        <table className="data">
          <thead>
            <tr>
              <th>الرمز</th><th>الخدمة</th><th>الحالة</th><th/>
            </tr>
          </thead>
          <tbody>
            {local.map((svc) => {
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: 5, justifyContent: 'flex-end' }}>
                      ...
                      <RowMenu
                        ...
                        onDelete={() => setEditModal({ open: true, service: svc })}
                      />
                    </div>
                  </td>
                </tr>
