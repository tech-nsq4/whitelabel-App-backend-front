...
function ClinicDropdown({ clinics, value, onChange }) {
  ...
  const selected = value === 'all'
    ? { label: 'كل العيادات' }
    : (() => { const c = clinics.find(c => String(c.id) === String(value)); return c ? { label: c.name?.ar || c.name } : { label: 'كل العيادات' } })()
  ...
  return (
    <div className="cdd-wrap" ref={ref}>
      <button className={`cdd-trigger${open ? ' open' : ''}`} onClick={() => setOpen(v => !v)}>
        <Building2 size={14} strokeWidth={1.8} className="cdd-icon" />
        <span className="cdd-label">{selected.label}</span>
        <ChevronDown size={13} strokeWidth={2} className={`cdd-arrow${open ? ' open' : ''}`} />
      </button>
      {open && (
        <div className="cdd-menu">
          <div className="cdd-menu-inner">
            <div className={`cdd-option${value === 'all' ? ' active' : ''}`} onClick={() => pick('all')}>
              <span>كل العيادات</span>
              {value === 'all' && <Check size={13} strokeWidth={2.5} className="cdd-check" />}
            </div>
            ...
            {clinics.map(c => {
              ...
              return (
                <div key={c.id} className={`cdd-option${isActive ? ' active' : ''}`} onClick={() => pick(c.id)}>
                  <span className="cdd-option-label">{nameAr}</span>
                  {isActive && <Check size={13} strokeWidth={2.5} className="cdd-check" />}
                </div>
              )
            })}
          </div>
...
export default function CalendarHeader({
  currentDate, currentDateValue, view,
  clinics, clinicFilter,
  dateInputRef, onPrev, onNext, onToday, onDateClick, onDateChange,
  onViewChange, onClinicChange, onNewBooking, onExport,
    ...
    <>
      <div className="page-head calendar-page-head">
        ...
        <div className="page-actions">
          <div className="seg">
            {VIEW_OPTIONS.map(v => (
              <div key={v} className={`seg-btn${view === v ? ' active' : ''}`} onClick={() => onViewChange(v)}>{v}</div>
            ))}
          </div>
        ...
        <div className="calendar-toolbar-inner">
          ...
          <div className="calendar-date-controls">
            ...
            <div style={{ position: 'relative' }}>
              <button onClick={onDateClick} className="cal-date-btn"
            ...
            <button className="btn btn-g cal-today-btn" onClick={onToday}>اليوم</button>
          </div>
          ...
          <ClinicDropdown clinics={clinics} value={clinicFilter} onChange={onClinicChange} />
