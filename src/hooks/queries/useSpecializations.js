...
import { getSubSpecializationsApi, createSubSpecializationApi, updateSubSpecializationApi, deleteSubSpecializationApi } from '../../api/sub-specializations.api'
...
export function useSubSpecializations(specializationId) {
  return useQuery({
    ...
    queryFn: () => getSubSpecializationsApi(specializationId).then(r => r.data.data || []),
  })
}
...
export function useCreateSubSpecialization() {
  ...
  return useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: SUB_SPECIALIZATIONS_KEY }),
  })
}
